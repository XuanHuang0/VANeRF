...
def vert_seal(mesh_to_seal):
    '''
    Seal MANO hand wrist to make it wathertight.
    An average of wrist vertices is added along with its faces to other wrist vertices.
    '''
    circle_v_id = torch.LongTensor([108, 79, 78, 121, 214, 215, 279, 239, 234, 92, 38, 122, 118, 117, 119, 120])
    center = (mesh_to_seal[:,circle_v_id, :]).mean(1).unsqueeze(1)
    mesh_seal = torch.cat([mesh_to_seal, center],dim=1)
...
def get_360cameras(headpose, focal, trans, sc_factor, im_w, im_h, znear, zfar, n_frames=90):
    ...
    for idx in range(n_frames):
        ...
        # print(theta1)
        idx=idx+0.0001
        if (idx>0 or idx==0) and (idx<n_frames/10 or idx==n_frames/10):
            theta0 = theta0 + 5.0 * np.pi*0.1 / n_frames
            theta1 = theta1 + 5.0 * np.pi*0.1 / n_frames
        elif (idx<n_frames*3/10) and (idx>n_frames/10):
            theta0 = theta0 - 5.0 * np.pi*0.1 / n_frames
        elif (idx<n_frames*5/10) and (idx>n_frames*3/10):
            theta1 = theta1 - 5.0 * np.pi*0.1 / n_frames
        elif (idx<n_frames*7/10) and (idx>n_frames*5/10):
            theta0 = theta0 + 5.0 * np.pi*0.1 / n_frames
        elif (idx<n_frames*9/10) and (idx>n_frames*7/10):
            theta1 = theta1 + 5.0 * np.pi*0.1 / n_frames
        elif idx>n_frames*9/10 or idx==n_frames*9/10:
            theta0 = theta0 - 5.0 * np.pi*0.1 / n_frames
            theta1 = theta1 - 5.0 * np.pi*0.1 / n_frames

        theta0 = theta0 + 2.0 * np.pi / n_frames
...
def feat_sample(feat, uv, mode='bilinear', padding_mode='border', align_corners=True):
    ...
    '''
    uv = uv[:, :, None]
    feat = thf.grid_sample( #import torch.nn.functional as thf
        feat,
        uv,
        mode=mode,
...
def compute_error(inter_loss=None,out_nerf=None, vggloss=None, lambdas={}):
    err_dict = {}
    err_dict.update(compute_error_nerf(out_nerf, lambdas, vggloss))
    if 'uv_rendered_dict' in out_nerf.keys() and lambdas["lambda_l1_uv"]!=0:
        err_dict.update(compute_error_uv_renderer(out_nerf, lambdas, vggloss))
    # print(inter_loss)
    # print(err_dict)
...
def compute_error_uv_renderer(out_nerf, lambdas, vggloss):

    uv_render_rgb=out_nerf['uv_rendered_dict']['fake_rgb']
    if 'fake_segm' in out_nerf['uv_rendered_dict'].keys():
        uv_render_segm=out_nerf['uv_rendered_dict']['fake_segm']

    # print(out_nerf['uv_rendered_dict'])

    
    err_dict = {}
    lambda_l1 = lambdas.get("lambda_l1_uv", 10.0)
    lambda_l1_msk = lambdas.get("lambda_msk_uv", 1.0)
    lambda_vgg = lambdas.get("lambda_vgg_uv", 0.1)

    loss_rbg = 0.0
    losses = pix_loss(uv_render_rgb, out_nerf["tar_img_all"], {"l1": lambda_l1})
    loss_rbg += losses["l1"]
    err_dict["uv_render_rgb"] = loss_rbg

    if 'fake_segm' in out_nerf['uv_rendered_dict'].keys():
        loss_segm = 0.0
        losses = pix_loss(uv_render_segm, out_nerf["tar_img_mask_all"].int(), {"l1": lambda_l1_msk})
        loss_segm += losses["l1"]
        err_dict["uv_render_segm"] = loss_segm
    ...
    if vggloss is not None:
        loss_vgg = 0.0
        loss_vgg += lambda_vgg * vggloss(uv_render_rgb, out_nerf["tar_img_all"])
        if loss_vgg > 0.0:
            err_dict["uv_render_vgg"] = loss_vgg
...
def compute_error_nerf(out_nerf, lambdas, vggloss):
    ...
    if "alpha" in out_nerf and 'tar_alpha' in out_nerf and lambda_mloss > 0.0:
        ...
        err_dict["mask_loss_c"] = lambda_mloss*thf.mse_loss(
            out_nerf['alpha'].clip(-0.001, 1.0).squeeze(),
    ...
    if "alpha_fine" in out_nerf and 'tar_alpha' in out_nerf and lambda_mloss > 0.0:
        ...
        err_dict["mask_loss_f"] = lambda_mloss*thf.mse_loss(
            out_nerf['alpha_fine'].clip(-0.001, 1.0).squeeze(),
            out_nerf['tar_alpha'].squeeze())

    if vggloss is not None:
        loss_vgg = 0.0
        if "tex_cal" in out_nerf:
...
def pix_loss(src, tar, w_losses={"l1": 1.0},vis_img=None):
    losses = {}
    ...
    for k, v in w_losses.items():
        ...
        if k == "l1":
            ...
            losses[k] = v * (src - tar).abs()
            if vis_img!=None:
                losses[k][vis_img==0]=losses[k][vis_img==0]*5
            losses[k] = losses[k].mean()
        elif k == "l2":
            losses[k] = v * (src - tar).pow(2.0)
            if vis_img!=None:
                losses[k][vis_img==0]=losses[k][vis_img==0]*5
            losses[k] = losses[k].mean()
        elif k == "lp":
            losses[k] = v * ((src - tar).abs() + 1e-4).pow(0.4)
            if vis_img!=None:
                losses[k][vis_img==0]=losses[k][vis_img==0]*5
            losses[k] = losses[k].mean()
        elif "l1top" in k:
