...
def get_visibility(xy, z, faces):
    ...
    rasterizer = Pytorch3dRasterizer(image_size=256)
...
def cal_vert_sdf_batch(verts, faces, points):
    ...
    verts_r=seal(verts[:,:778,:])
    verts_l=seal(verts[:,778:,:])
    verts=torch.cat([verts_r,verts_l],dim=1)

    face_vertices = index_vertices_by_faces(verts, faces.squeeze(0))
    residues, pts_ind, _ = point_to_mesh_distance(points, face_vertices)
    pts_dist = torch.sqrt(residues+1e-6)
    pts_signs = -2.0 * (check_sign(verts, faces.squeeze(0), points).float() - 0.5)
    pts_sdf = (pts_dist * pts_signs)
    return pts_sdf

def cal_vert_sdf_batch_single(verts, faces, points):
...
def compute_vis(vert_xy,vert_z, smpl_faces):
    xy=vert_xy.squeeze(0)
    z=vert_z.squeeze(0)

    smpl_vis = get_visibility(xy, z, smpl_faces.squeeze(0))
    return smpl_vis.unsqueeze(0)


# def compute_vis2(vert_xy,vert_z, smpl_faces):
#     xy=vert_xy
#     z=vert_z
#     smpl_vis = get_visibility(xy, z, smpl_faces)
#     return smpl_vis

def cal_vis_sdf_batch(verts, faces, points, vert_xy, vert_z):
    ...
    face_vertices = index_vertices_by_faces(verts.to(faces.device), faces.squeeze(0))
    residues, pts_ind, _ = point_to_mesh_distance(points, face_vertices)
    pts_dist = torch.sqrt(residues+1e-6)
    pts_signs = -2.0 * (check_sign(verts, faces.squeeze(0), points).float() - 0.5)
    pts_sdf = (pts_dist * pts_signs)

    vert_vis=compute_vis(vert_xy.float(), vert_z.float(), faces)
    vis = index_vertices_by_faces(vert_vis.to(faces.device), faces.squeeze(0))
    closest_vis = torch.gather(vis, 1, pts_ind[:, :, None, None].expand(-1, -1, 3, 1)).view(-1, 3, 1)
    closest_triangles = torch.gather(face_vertices, 1,
                                     pts_ind[:, :, None, None].expand(-1, -1, 3, 3)).view(-1, 3, 3)
    closest_face = torch.gather(faces.unsqueeze(-1), 1, pts_ind[:, :, None, None].expand(-1, -1, 3, 1)).view(-1, 3, 1)

    bary_weights = barycentric_coordinates_of_projection(points.view(-1, 3), closest_triangles)
    pts_vis = (closest_vis * bary_weights[:, :, None]).sum(1).unsqueeze(0).ge(1e-1)

    return pts_sdf,pts_vis.view(Bsize, -1, 1).to(pts_sdf.device),vert_vis.to(pts_sdf.device), closest_face.unsqueeze(0).squeeze(-1).to(pts_sdf.device)

def cal_vis_sdf_batch_single(verts, faces, points, vert_xy, vert_z):
...
def cal_vis_batch(verts, faces, points, vert_xy, vert_z):
    ...
    vert_vis=compute_vis(vert_xy, vert_z, faces)
